...
beforeAll(async () => {
  ...
  const user = await createTestUser({ email: 'respuser@test.com', password: 'P@ssw0rd123!' });
  ...
  const userRes = await request(app).post('/api/auth/login').send({ email: 'respuser@test.com', password: 'P@ssw0rd123!' });
