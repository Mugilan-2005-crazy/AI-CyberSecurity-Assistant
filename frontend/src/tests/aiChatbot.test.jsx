...
import AIChatbot from '../pages/modules/AIChatbot.jsx';
...
describe('AI Chatbot', () => {
  ...
  it('renders chat interface with greeting', () => {
    renderWithI18n(<AIChatbot />);
    expect(screen.getByText(/cyber security assistant/i)).toBeInTheDocument();
  });
  ...
  it('shows loading state while waiting for AI response', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/cyber assistant is thinking/i)).toBeInTheDocument();
    });
  });
});